...
/**
 * Interface representing a move in a Quiz game.
 * - `playerID`: The ID of the player making the move.
 * - `answer`: The answer selected by the player.
 * - `questionId`: The ID of the question being answered.
 * - Or `type`: 'START_GAME' to start the game.
 * - `timeExpired`: Indicates that the timer for the question has expired.
 */
export interface QuizMove extends BaseMove {
  playerID?: string;
  answer?: string;
  questionId?: string;
  type?: 'START_GAME';
...
/**
 * Interface representing the state of a Quiz game, which includes:
 * - `moves`: A list of moves (answers) made in the game.
 * - `currentQuestion`: The current question being asked.
 * - `questions`: Array of questions in the quiz.
 * - `scores`: Object mapping player IDs to their scores.
 * - `topic`: The selected topic for the quiz.
 * - `player1`: The ID of the first player (host).
 * - `readyToStart`: Whether the host has started the game.
 */
export interface QuizGameState extends WinnableGameState {
  moves: ReadonlyArray<QuizMove>;
...
/**
 *  * Interface extending the request body when fetching a singular
 *  multiple-choice (MC) question.
 * - `category`: The category of the question
 * - `difficulty` (optional): The difficulty level of the question, which must be one of:
 *   - 'easy'
 *   - 'medium'
 *   - 'hard'
 */
export interface GetMCQuestionRequest extends Request {
  body: {
    category: string;
 ...
 * - `topic`: Optional topic for quiz games.
 */
