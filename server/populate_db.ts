...
async function skillCreate(skillName: string): Promise<DatabaseSkill> {
  if (skillName === '') {
...
async function dockerEntryCreate(username: string, score: number): Promise<DatabaseLeaderboardEntry> {
  if (username === '') {
    throw new Error('Invalid username');
  }
  const entry: DatabaseLeaderboardEntry = { username, skill: 'Docker', score };
  return await LeaderboardModel.create(entry);
}

async function populateDockerLeaderboard() {
  ...
  // Clear any existing Docker leaderboard entries
  await LeaderboardModel.deleteMany({ skill: 'Docker' });
  console.log('Cleared existing Docker leaderboard entries.');

  try {
    await dockerEntryCreate('alice', 250);
    console.log('Created entry for alice with score 250');

    await dockerEntryCreate('bob', 300);
    console.log('Created entry for bob with score 300');

    await dockerEntryCreate('charlie', 150);
    console.log('Created entry for charlie with score 150');

    await dockerEntryCreate('dave', 275);
    console.log('Created entry for dave with score 275');

    await dockerEntryCreate('eve', 225);
    console.log('Created entry for eve with score 225');

    await dockerEntryCreate('frank', 325);
    console.log('Created entry for frank with score 325');

    await dockerEntryCreate('grace', 200);
    console.log('Created entry for grace with score 200');

    await dockerEntryCreate('heidi', 180);
    console.log('Created entry for heidi with score 180');

    await dockerEntryCreate('ivan', 320);
    console.log('Created entry for ivan with score 320');

    await dockerEntryCreate('judy', 290);
    console.log('Created entry for judy with score 290');

    console.log('Docker leaderboard populated successfully.');
  } catch (error) {
    console.error('Error populating Docker leaderboard:', error);
  } finally {
...
const populate = async () => {
  try {
    ...
    await LeaderboardModel.deleteMany({ skill: 'Docker' });
    await dockerEntryCreate('monkeyABC', 250);
    await dockerEntryCreate('hamkalo', 300);
    await dockerEntryCreate('sana', 150);
    await dockerEntryCreate('ihba001', 275);
    await dockerEntryCreate('saltyPeter', 225);
    await dockerEntryCreate('abhi3241', 325);
    await dockerEntryCreate('Joji John', 200);
    await dockerEntryCreate('azad', 180);
    await dockerEntryCreate('mackson3332', 320);
    await dockerEntryCreate('alia', 290);
    await dockerEntryCreate('elephantCDE', 275);

    const s1 = await skillCreate('Docker');
    const s2 = await skillCreate('Linux');
    const s3 = await skillCreate('nodeJS');
    const s4 = await skillCreate('React');
    const s5 = await skillCreate('Django');
